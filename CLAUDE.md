...
**Resolver** (`src/resolver.rs`) is the single entry point that turns an `Id` into a `ResolvedTarget` (Root/Project/File/Node/Comment). It wraps the cache, the `SynthState`, and a lazily-built `NodeIndex` (only constructed when a bare-node lookup actually needs it). Comment ids (`file:N:comm:M`) resolve by reading the `.comments.json` sidecar; `node-info` and `comments` consume that variant — every other command rejects comment ids with a hint.
