...
) -> Result<models::InlineObject22, Error<GetTeamWebhooksError>> {
    let uri_str = format!(
...
pub async fn get_webhook_requests(
...
) -> Result<models::InlineObject23, Error<GetWebhookRequestsError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::InlineObject23`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::InlineObject23`")))),
        }
    } else {
        let content = resp.text().await?;
...
) -> Result<models::InlineObject21, Error<GetWebhooksError>> {
