...
) -> Result<models::InlineObject27, Error<GetLocalVariablesError>> {
    let uri_str = format!(
...
) -> Result<models::InlineObject28, Error<GetPublishedVariablesError>> {
    let uri_str = format!(
...
pub async fn post_variables(
...
) -> Result<models::InlineObject29, Error<PostVariablesError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::InlineObject29`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::InlineObject29`")))),
        }
    } else {
        let content = resp.text().await?;
