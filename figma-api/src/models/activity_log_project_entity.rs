/*
 * Figma API
 *
 * This is the OpenAPI specification for the [Figma REST API](https://developers.figma.com/docs/rest-api/).  Note: we are releasing the OpenAPI specification as a beta given the large surface area and complexity of the REST API. If you notice any inaccuracies with the specification, please [file an issue](https://github.com/figma/rest-api-spec/issues).
 *
 * The version of the OpenAPI document: 0.42.0
 * Contact: support@figma.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// ActivityLogProjectEntity : A project that a collection of Figma files are grouped under
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ActivityLogProjectEntity {
    /// The type of entity.
    #[serde(rename = "type")]
    pub r#type: Type,
    /// Unique identifier of the project.
    #[serde(rename = "id")]
    pub id: String,
    /// Name of the project.
    #[serde(rename = "name")]
    pub name: String,
}

impl ActivityLogProjectEntity {
    /// A project that a collection of Figma files are grouped under
    pub fn new(r#type: Type, id: String, name: String) -> ActivityLogProjectEntity {
        ActivityLogProjectEntity { r#type, id, name }
    }
}
/// The type of entity.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "project")]
    Project,
}

impl Default for Type {
    fn default() -> Type {
        Self::Project
    }
}
